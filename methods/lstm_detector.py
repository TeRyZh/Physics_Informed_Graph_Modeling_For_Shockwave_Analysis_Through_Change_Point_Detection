"""
LSTM-based Change Point Detector with Wavelet-Enhanced Labeling

This module implements an LSTM detector that can be trained with:
- Moving window labels (baseline)
- Wavelet-enhanced labels (RECOMMENDED)
- Wavelet-only labels
"""
...
class LSTMChangePointModel(nn.Module):
    """LSTM model for change point detection."""
    
    def __init__(self, input_size: int, hidden_size: int = 64, num_layers: int = 2, 
                 dropout: float = 0.3):
        super(LSTMChangePointModel, self).__init__()
...
class LSTMChangePointDetector:
    """LSTM-based change point detector with wavelet-enhanced training."""
    ...
    def __init__(self, sequence_length: int = 100, hidden_size: int = 64, 
        ...
        """
        Initialize LSTM change point detector.
    ...
    def train(self, train_trajectories: Dict, val_trajectories: Dict = None,
        ...
        """
        Train the LSTM model with wavelet-enhanced labels.
        ...
        print(f"\n{'='*80}")
        print(f"Training LSTM with labeling mode: {self.labeling_mode}")
        print(f"{'='*80}")
        ...
        self.model = LSTMChangePointModel(
            input_size=input_size,
    ...
    def detect(self, trajectory: Dict) -> Tuple[List[int], Dict]:
        ...
        dataset = WaveletEnhancedDataset(
            ...
            feature_engineering=True,
            normalize=False,
            labeling_mode='moving_window_only'  # For detection, don't use wavelet labels
        )
...
if __name__ == "__main__":
    ...
    print("LSTM Change Point Detector Example")
    print("="*80)
    ...
    # Train LSTM with wavelet-enhanced labels
    print("\nTraining LSTM with wavelet-enhanced labels...")
    lstm = LSTMChangePointDetector(
        sequence_length=40,
